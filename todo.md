...
## Bug - Guest Assignment Synchronization
- [ ] Employee guest list not showing table assignments made on couple side
- [ ] Investigate if assignments are being saved to database correctly
- [ ] Check if employee GuestList is querying the correct data
- [ ] Verify real-time synchronization between couple and employee interfaces


## Bug - Guest Assignment Synchronization (FIXED)
- [x] Investigate why guest table assignments from couple side don't appear on employee side
- [x] Identify root cause of data synchronization issue (seats table was empty)
- [x] Fix the data synchronization between couple and employee interfaces
- [x] Test and verify guest assignments are visible on both sides
- [x] Add seats.list procedure to query seats by floorPlanId
- [x] Update CoupleSeatingV2 to create seat records when assigning guests
- [x] Update GuestList to query and display seat information
- [x] Verify both couple and employee interfaces show matching data
